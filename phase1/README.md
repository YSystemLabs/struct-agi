...
说明：Phase 1 的 Step 1/2 代码与实验先形成了当前工程基线；根目录 [docs/draft/可组合信息论.md](../docs/draft/可组合信息论.md) 是后续对这些工程实践的理论综合。当前根目录 [experiments/multi_preorder_minimal_validation](../experiments/multi_preorder_minimal_validation/多预序对象发现最小验证实验.md) 只应视为理论启发的探索性最小验证包，用于筛选多预序路线是否值得继续保留，而不应视为主理论的正式 Phase A 实验包；并行候选路线还包括根目录实验目录下的 [experiments/softWTA_softHebb_minimal_validation/SoftWTA 关系原型发现最小验证实验.md](../experiments/softWTA_softHebb_minimal_validation/SoftWTA%20关系原型发现最小验证实验.md)。[docs/structure-miner/](docs/structure-miner/README.md) 仅保留旧实验路线的 brainstorming 补充，不作为正式理论或正式实验指导。
...
- `scripts/`：阶段内会反复使用的工具脚本与辅助渲染/批处理程序。
...
- 并行 SoftWTA 探索草案：[../experiments/softWTA_softHebb_minimal_validation/SoftWTA 关系原型发现最小验证实验.md](../experiments/softWTA_softHebb_minimal_validation/SoftWTA%20关系原型发现最小验证实验.md)
