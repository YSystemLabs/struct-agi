...
from phase1.src.step1.data.models import Alignment, CandidateConstraint, CandidateTransform, SearchStats
...
from phase1.src.step1.runner.batch_runner import build_summary, run_step1_batch
from phase1.src.step1.runner.task_runner import _build_search_stats, _materialize_candidate_sets, run_task


class RunnerTests(unittest.TestCase):
    def test_single_task_run_writes_debug_artifacts(self) -> None:
        task = load_step1_train_tasks()[0]
    ...
    def test_batch_runner_only_consumes_train_tasks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            attributions = run_step1_batch(temp_dir)
            self.assertEqual(12, len(attributions))
            self.assertTrue(all(item.task_id.startswith(("Copy", "Center")) for item in attributions))
