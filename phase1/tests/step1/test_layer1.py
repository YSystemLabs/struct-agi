...
from phase1.src.step1.layer1.perception import build_segmentation_plan, perceive_grid
from phase1.src.step1.layer1.relations import extract_relations
from phase1.tests.step1.fixtures import diag_touch_grid
...
class Layer1Tests(unittest.TestCase):
    ...
    def test_perception_output_schema_is_stable(self) -> None:
        ...
        self.assertEqual({"plan_id", "method", "objects", "relations"}, set(first_plan.keys()))
        first_object = first_plan["objects"][0]
        self.assertEqual({"id", "pixels", "bbox", "attrs"}, set(first_object.keys()))
    ...
    def test_perceive_grid_returns_three_stable_plans(self) -> None:
        output = perceive_grid([[1, 0], [0, 1]])
        self.assertEqual(["cc4", "cc8", "whole_grid"], [plan.plan_id for plan in output.segmentation_plans])
