...
**理论与实验包说明**：Phase 1 的 Step 1/2 工程工作先形成了当前单任务基线；根目录下的 [可组合信息论](docs/draft/可组合信息论.md) 是对这些工程实践的后续理论综合。当前已整理成可复现实验包的是 [experiments/multi_preorder_minimal_validation](experiments/multi_preorder_minimal_validation/多预序对象发现最小验证实验.md)，但它目前只应视为理论启发下的探索性最小验证包，用于筛选多预序路线是否值得继续保留，而不应视为主理论的正式 Phase A 实验包。并行的自动对象发现探索还包括 [experiments/softWTA_softHebb_minimal_validation/SoftWTA 关系原型发现最小验证实验.md](experiments/softWTA_softHebb_minimal_validation/SoftWTA%20关系原型发现最小验证实验.md)；[phase1/docs/structure-miner](phase1/docs/structure-miner/README.md) 仅保留旧实验路线的 brainstorming 补充，不作为正式理论或正式实验指导。
...
│   └── softWTA_softHebb_minimal_validation/
│       ├── SoftWTA 关系原型发现最小验证实验.md
│       └── 和multi-preder方法的关系.md
...
| [SoftWTA 关系原型发现最小验证实验](experiments/softWTA_softHebb_minimal_validation/SoftWTA%20关系原型发现最小验证实验.md) | 并行探索中的自动对象发现分层最小验证草案 |
